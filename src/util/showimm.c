  ...
  const char *imm_powers[] =
{
  ...
  for (;;) {
    ...
    sprintbit((long) player.specials2.godlevel, imm_powers, buf2);

    printf("%5d. ID: %5ld (%c) [%3d %s] %-16s Race(%s) Room[%5d]\n"
           "       Remort[%d] Host[%-15s] Gold[%9d] Bank[%9d]\n"
           "       Created: %s"           
           "       Imm Powers[ %s]\n\n", ++num,
           player.specials2.idnum, sexname, player.level,
           classname, player.name, racename, player.specials2.load_room,
           player.specials2.remorts, player.host, player.points.gold,
           player.points.bank_gold, buf1, buf2);
