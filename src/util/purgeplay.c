...
void purge(char *filename)
{
  ...
  for (;;) {
    ...
    *reason = '\0';
    ...
    if (okay && player.level <= LEVEL_DEITY && player.specials2.remorts <= 0) {

      if (!(player.specials2.act & PLR_CRYO)) {
	if (player.level == 1)		timeout = 30;	/* Lev   1 :  30 days */
	else if (player.level <= 9)	timeout = 180;	/* Lev 2-9 : 180 days */
	else if (player.level <= LEVEL_DEITY)
	  timeout = 1000;				/* Lev 10-110: 1000 days */
      } else
	timeout = 1000;
