...
void convert(char *filename, int clan)
{
  FILE *infile;
  FILE *outfile;
  int i;
  ...
  for (;;) {
    ...
    if(player.specials2.clan == clan) {
      fprintf(outfile, "%s %d", player.name, player.specials2.clanlevel);
	e++;
    }
    if(e == 2)  {
	fprintf(outfile, "\n");
...
int main(int argc, char **argv)
{
  ...
  if (argc != 3)
    printf("Usage: %s playerfile-name clan\n", argv[0]);
  else {
      sprintf(crap, "%s", argv[2]);
	printf("%s\n", crap);

     scanf(crap, "%d", &clan);
	printf("%d\n", clan);
    printf("Doing a search for all players in clan %d.\n", clan);
/*    convert(argv[1], clan);  */
  }
